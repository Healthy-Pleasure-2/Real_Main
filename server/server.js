...
app.get("/todo/:id", async (req, res) => {
  try {
    ...
    const data = await fs.promises.readFile("./db.json", "utf8");
    const jsonData = JSON.parse(data);
    console.log(jsonData.todo, userId)
    // res.json(jsonData.todo);
    // jsonData.todo에서 date 필드를 기반으로 필터링하여 선택한 날짜에 해당하는 할 일 목록을 반환합니다.
    const userIdFilteredTodoList = jsonData.todo.filter((item) => item.userId === userId)
    console.log(userIdFilteredTodoList);
    console.log(fullDate);
    const filteredTodoList = userIdFilteredTodoList.filter((item) => { return item.date === fullDate })
    console.log("filter된 데이터: ", filteredTodoList, fullDate)
...
//         res
//           .status(200)
//           .json({ message: "해당 날짜에 데이터를 찾았습니다.", matchingDate });
//       } else {
